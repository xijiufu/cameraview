...
class Camera2 extends CameraViewImpl {
    ...
    void startVideoRecord(MediaRecorder mediaRecorder, VideoConfig videoConfig, String videoPath) {
        ...
        //帧率
        mediaRecorder.setVideoFrameRate(videoConfig.getVideoFrameRate());
        //视频编码格式
        mediaRecorder.setVideoEncoder(videoConfig.getVideoCodec());
        //音频编码格式
        mediaRecorder.setAudioEncoder(videoConfig.getAudioCodec());

        //屏幕方向
        switch (mSensorOrientation) {
            case SENSOR_ORIENTATION_DEFAULT_DEGREES:
                mediaRecorder.setOrientationHint(
                        DEFAULT_ORIENTATIONS.get(mDisplayOrientation));
                break;
            case SENSOR_ORIENTATION_INVERSE_DEGREES:
                mediaRecorder.setOrientationHint(
                        INVERSE_ORIENTATIONS.get(mDisplayOrientation));
                break;
        }

        //分辨率
        android.util.Size videoSize = getOptimalSize(mVideoSizes, videoConfig.getWidth(),
